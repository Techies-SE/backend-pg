...
router.get("/:hn_number/:lab_test_id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // ✅ Get AI prediction details for this lab test
    let aiPredictionDetails = [];
    if (recommendationsQuery.rows.length > 0) {
      const recommendationId = recommendationsQuery.rows[0].recommendation_id;

      const aiDetailsQuery = await client.query(
        `
          SELECT
            test_name,
            best_class,
            probability
          FROM ai_prediction_details
      ...
      aiPredictionDetails = aiDetailsQuery.rows;
    }
    ...
    const patient = {
      ...
      ai_prediction_details: aiPredictionDetails, // ✅ Added here
    };
